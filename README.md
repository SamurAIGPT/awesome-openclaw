...
| [Omnicall](https://omnicall.gocreativeai.com) | Keyless pay-per-call AI gateway — 248 LLMs (GPT, Claude, Gemini, Grok, DeepSeek, Llama, Qwen, Mistral) plus image/video/voice/music generation and live crypto, DeFi, markets, web-search and research tools, through one OpenAI-compatible endpoint. Pay per call in USDC via x402 on Base/Solana — no API key, free tier. Config: `{"mcpServers":{"omnicall":{"url":"https://omnicall.gocreativeai.com/mcp"}}}` or `npx -y omnicall-mcp`. Listed on the Official MCP Registry as `com.gocreativeai.omnicall/agent-tools`. |
